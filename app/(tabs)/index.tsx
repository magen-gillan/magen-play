...
export default function HomeScreen() {
  ...
  const [sheet, setSheet] = useState<"none" | "settings" | "edit" | "crop" | "loop">("none");
  ...
  const filtered = useMemo(() => TRACKS.filter((track) => `${track.title} ${track.artist} ${track.album}`.toLowerCase().includes(query.toLowerCase())), [query]);

  const playTrack = (track: Track) => { setCurrent(track); setEditedTitle(track.title); setPlaying(true); };
    ...
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      ...
      <FlatList data={filtered} keyExtractor={(item) => item.id} showsVerticalScrollIndicator={false} contentContainerStyle={styles.list} renderItem={({ item }) => (
        <Pressable onPress={() => playTrack(item)} style={({ pressed }) => [styles.trackRow, { backgroundColor: item.id === current.id ? colors.surface : "transparent" }, pressed && styles.pressed]}>
          ...
          <Pressable onPress={() => setSheet("edit")} style={({ pressed }) => [styles.moreButton, pressed && styles.pressed]}><IconSymbol name="ellipsis" size={20} color={colors.muted} /></Pressable>
        </Pressable>
      )} />

      <Pressable onPress={() => setSheet("none")} style={[styles.miniPlayer, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      ...
      <Modal visible={sheet !== "none"} transparent animationType="slide" onRequestClose={closeSheet}>
        <View style={styles.modalBackdrop}><View style={[styles.sheet, { backgroundColor: colors.background }]}>
          ...
          <View style={styles.sheetHeader}><Text style={[styles.sheetTitle, { color: colors.foreground }]}>{sheet === "settings" ? "تخصيص التجربة" : sheet === "edit" ? "تحرير بيانات الأغنية" : sheet === "crop" ? "قص الأغنية" : "تكرار مقطع A-B"}</Text><Pressable onPress={closeSheet}><IconSymbol name="xmark" size={24} color={colors.muted} /></Pressable></View>
          {sheet === "settings" && <View>
